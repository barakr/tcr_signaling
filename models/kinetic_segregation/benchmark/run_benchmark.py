...
def main():
    ...
    # NB: the run seed is fixed at 42 inside `run()`, which builds the argv.
        ...
        for gs in grid_sizes:
            for ns in step_counts:
                for cfg in CONFIGS:
                    ...
                    t_cpu, d_cpu = run(
                        tmp_path, grid_size=gs, n_steps=ns, config_name=cfg, use_gpu=False
                    )
                    ...
                    try:
                        t_gpu, d_gpu = run(
                            tmp_path, grid_size=gs, n_steps=ns, config_name=cfg, use_gpu=True
                        )
                        ...
                        print(
                            f"  GPU: {t_gpu:.3f}s  depl={row['depletion_gpu']}nm  "
    ...
    for r in results:
        ...
        print(
            f"{r['config']:<22} {r['grid_size']:>4} {r['n_steps']:>6} "
