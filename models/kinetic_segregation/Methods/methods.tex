  ...
  \begin{tabular}{@{}p{2.8cm}p{5.2cm}p{3.5cm}p{3.5cm}@{}}
    ...
    TCR--pMHC binding &
      $\displaystyle E_\text{TCR}(h)
        = -U_\text{assoc}\,
          \exp\!\Bigl(-\frac{(h-h_0^\text{TCR})^2}{2\sigma_\text{bind}^2}\Bigr)$ &
      $U_\text{assoc}=20\;k_BT$; \newline
      $\sigma_\text{bind}=3$~nm &
      Gaussian attractive well centred at the bond length $h_0^\text{TCR}$ \\[6pt]
...
centred at the resting TCR--pMHC bond length $h_0^\text{TCR}=13$~nm---%
\emph{not} at $h=0$.  A bound TCR is equally disfavoured when the membranes
are too far apart and when they are squeezed closer than the bond allows:

\begin{equation}
  \label{eq:tcr}
  E_\text{TCR}(h) \;=\; -U_\text{assoc}\,
    \exp\!\left(-\frac{(h-h_0^\text{TCR})^2}{2\,\sigma_\text{bind}^2}\right)
\end{equation}
...
\end{equation}
with $\alpha_\text{safety}=0.05$ (\texttt{MAX\_BENDING\_DE\_PER\_STEP}), the
maximum typical bending $\Delta E$ per grid point per step.  A second reduction
then applies: $\Delta t \leftarrow \min(\Delta t,\;\ell^2/(8 D_\text{mol}))$,
where $\ell$ is the smallest resolved length among $\Delta x$, $\sigma_r$
(gaussian binding with pMHC present) and the molecular repulsion cutoff.  This
keeps a proposed lateral move small relative to every feature it must resolve.  The molecular and
...
\begin{itemize}
  ...
  \item pMHC ligands are placed on the APC surface---either uniformly
    ...
    (``inner-circle'' mode).  Uniform placement is the default; note that
    inner-circle is required for the centre-referenced depletion metrics to be
    meaningful.  They remain static throughout
    the simulation.
...
\begin{table}[ht]
  ...
  \begin{tabular}{@{}lll@{}}
    ...
    Grid spacing        & $\Delta x$            & $L / N_\text{grid} \approx 31.25$~nm
                                                  (but see \S\ref{sec:resolution}) \\
    Number of TCR       & $N_\text{TCR}$        & 125 \\
    Number of CD45      & $N_\text{CD45}$       & 500 \\
    Number of pMHC      & $N_\text{pMHC}$       & auto: $300/\mu$m$^2$ over the
                                                  placement area \\
    ...
    Safety factor       & $\alpha_\text{safety}$ & 0.05 \\
...
%% ----------------------------------------------------------------
\subsection{Grid resolution and the pMHC influence field}
\label{sec:resolution}

In \texttt{gaussian} binding mode the TCR well at each cell is scaled by a pMHC
influence weight $W=\min\!\bigl(1,\sum_k \exp(-r_k^2/2\sigma_r^2)\bigr)$, summed
over pMHC within $3\sigma_r$ of the cell centre, with $\sigma_r=2$~nm by default.

This makes $\Delta x$ a physical parameter rather than a discretisation choice.
When $\Delta x \gg \sigma_r$ no cell centre lies within the cutoff, $W$ is
identically zero, and the attractive term vanishes---so a seeded contact simply
relaxes to $h_\text{init}$.  Measured at fixed physical time, the mean height
inside the contact disc ends at 63.6~nm for $\Delta x=41.7$~nm versus 20.2~nm for
$\Delta x=7.8$~nm.  Configurations intended to exercise binding should therefore
keep $\Delta x$ within roughly a factor of a few of $\sigma_r$; the behaviour
sweep in \texttt{experiments/ks\_behavior\_sweep/} uses $L=500$~nm with
$N_\text{grid}=100$ ($\Delta x=5$~nm) for this reason.
...
\subsection{Backend selection and reporting}

The GPU path is used by default when it is available, and
\texttt{--no-gpu} forces the CPU path.  The two are not numerically
identical: the GPU evaluates the energies in single precision and draws
from a Philox counter-based stream, while the CPU path uses double
precision and PCG64, so the two are statistically equivalent
realisations rather than the same trajectory.  Reported results must
therefore state which path produced them, and every run records the
backend both on \texttt{stderr} and as \texttt{diagnostics.backend}
(\texttt{"metal"} or \texttt{"cpu"}) in the output JSON.

Availability is determined solely by the flags and by whether a Metal
device and shader are present.  The shader is located relative to the
\emph{executable}, never the working directory: until 2026-08-11 the
lookup was working-directory relative, which made the backend --- and
hence the numerical result --- depend on the directory the caller
happened to invoke the binary from.  Where a run must not silently
degrade, \texttt{--require-gpu} makes an unavailable GPU backend a
fatal error instead of a fallback.
