...
./ks_gpu --time_sec 20 --rigidity_kT 20 --run-dir /tmp/test --require-gpu # fail if no GPU
```
...
The GPU and CPU paths differ in precision and RNG stream, so they give measurably
different results — on a small test case, `depletion_width_nm` 312.67 (Metal) against
283.91 (CPU). Every run therefore records the backend it used, both on stderr
(`Backend: metal`) and in the output JSON:

```json
"diagnostics": { "backend": "metal", ... }
```

The choice depends only on the flags and on whether Metal is available. It does **not**
depend on your working directory: the shader is located relative to the executable. It
was cwd-relative until 2026-08-11, which silently put every framework-driven sweep on
the CPU path — see `Status.md`.
