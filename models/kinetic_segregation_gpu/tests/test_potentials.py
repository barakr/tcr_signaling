"""Tests for C potential functions via ctypes, compared against Python reference."""
...
class TestTcrPotentialC:
    ...
    def test_matches_python(self, lib):
        """Compare against Python reference for several heights."""
        from models.kinetic_segregation.potentials import tcr_pmhc_potential as py_tcr

        for h in [0.0, 1.5, 3.0, 10.0, 35.0, 50.0]:
            c_val = lib.tcr_pmhc_potential(h, 20.0, 3.0)
            py_val = py_tcr(h, 20.0, 3.0)
...
class TestBendingEnergyDeltaC:
    def test_matches_python(self, lib):
        """Compare C bending_energy_delta against Python for random perturbations."""
        import numpy as np
        from models.kinetic_segregation.potentials import bending_energy_delta as py_delta
        ...
        for _ in range(20):
            ...
            h_tmp = h_np.copy()
            h_tmp[gi, gj] = new_val
            py_val = py_delta(h_tmp, kappa, dx, gi, gj, old_val, new_val)
            ...
            assert c_val == pytest.approx(py_val, abs=1e-10), (
                f"Mismatch at ({gi},{gj}): C={c_val}, Python={py_val}"
            )
