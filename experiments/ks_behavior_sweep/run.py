...
FIXED_ARGS = [
    ...
    str(SIM_TIME),
    "--grid_size",
    "100",  # doubled from 50 (keeps dx=5nm)
    "--patch_size",
    "500",  # doubled from 250
    "--n_tcr",
    "30",
    "--n_cd45",
    "30",
    ...
    "30",
    "--pmhc_radius",
    "21.0",  # physical radius unchanged
    "--pmhc_mode",
    "1",  # inner_circle
    "--step_mode",
    "brownian",
...
def run_single(
    kappa: float, dt: float, seed: int, run_dir: Path
...
def make_progress_plot(results_csv: Path, output: Path, done: int, total: int, elapsed: float):
    ...
    for idx, (mkey, mname) in enumerate(METRIC_NAMES):
        ...
        for ki, k_val in enumerate(k_show):
            ...
            for dt_val in dt_vals:
                ...
                ax.plot(
                    ts, means, ls, color=dt_color[dt_val], alpha=0.7, linewidth=1.2, label=label
                )
...
def main():
    ...
    if args.dry_run:
        ...
        for kappa, dt_sec, seed, rd in schedule[:10]:
            print(
                f"  K={kappa:6.2f}  dt={dt_sec:.2e}  seed={seed}  "
    ...
    if results_csv.exists():
        with open(results_csv) as f:
            reader = csv.DictReader(f)
            for row in reader:
                key = (float(row["rigidity_kT"]), float(row["dt_sec"]), int(row["seed"]))
                existing_run_keys.add(key)
        n_existing_rows = sum(1 for _ in open(results_csv)) - 1
        print(f"Loaded {len(existing_run_keys)} existing runs ({n_existing_rows} rows) from CSV")
    ...
    for idx, (kappa, dt_sec, seed, rd) in enumerate(schedule):
        ...
        if new_runs % 5 == 0 or new_runs == 1:
            ...
            print(
                f"  [{done_runs}/{total_runs}] {new_runs} new, "
        ...
        if now - last_plot >= PLOT_INTERVAL_SEC:
            make_progress_plot(
                results_csv, OUT_DIR / "progress_latest.png", done_runs, total_runs, now - t0
            )
    ...
    elapsed = time.time() - t0
    done_runs = len(existing_run_keys)
    make_progress_plot(results_csv, OUT_DIR / "progress_latest.png", done_runs, total_runs, elapsed)
    make_progress_plot(
        results_csv, OUT_DIR / "behavior_sweep_final.png", done_runs, total_runs, elapsed
    )

    # --- Summary ---
    print(f"\n{'=' * 90}")
    print(
        f"Completed: {done_runs}/{total_runs} runs "
